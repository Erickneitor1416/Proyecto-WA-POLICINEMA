...
fdescribe('ReviewsScreenComponent', () => {
  ...
  const mockFirestore = {
    collection: () => ({
      valueChanges: () => of([
        {date: '08/09/2023', movieId: 240, reviewTitle: 'Masterpiece', rating: 5, feedback: 'No words needed'},
      ]),
    }),
  };
  ...
  it('should fetch reviews data', () => {
    const movieId = 240;
    const spyValueChanges = spyOn(firestore.collection('reviews') as any, 'valueChanges').and.callThrough();
